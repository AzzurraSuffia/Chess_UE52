...
bool AChess_HumanPlayer::GetIsMyTurn() const
{
	return bisMyTurn;
}

void AChess_HumanPlayer::SetIsMyTurn(const bool flag)
{
		bisMyTurn = flag;
}
		...
		else
		{
			...
			{	
				...
				if (DestinationPiece == CurrPiece)
				{
					...
					if (GameMode->ChessBoard->MoveStack.Num() > 2)
					{
						//if the current chessboard state is different from the last move done, underline the tiles in current chessboard state
						if (GameMode->ChessBoard->CurrentChessboardState != GameMode->ChessBoard->MoveStack[GameMode->ChessBoard->MoveStack.Num() - 2])
						{
							if (GameMode->ChessBoard->CurrentChessboardState->To->GetTileColor() != 5)
							{
								GameMode->ChessBoard->CurrentChessboardState->To->SetTileColor(5);
							}
							if (GameMode->ChessBoard->CurrentChessboardState->From->GetTileColor() != 5)
							{
								GameMode->ChessBoard->CurrentChessboardState->From->SetTileColor(5);
							}
						}
						// if the current chessboard state is equal to the last move done, restore tile colors
						else
						{
							if (GameMode->ChessBoard->CurrentChessboardState->To->GetTileColor() == 5 || GameMode->ChessBoard->CurrentChessboardState->From->GetTileColor() == 5)
							{
								GameMode->ChessBoard->CurrentChessboardState->To->SetTileColor(5);
								GameMode->ChessBoard->CurrentChessboardState->From->SetTileColor(5);
							}
