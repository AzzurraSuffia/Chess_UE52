...
void AChess_GameMode::ManageEndOfGame(int32 Player, EResult GameResult)
{
	...
	if (GameResult == EResult::CHECKMATE)
	{
		...
		//highlight the king of the losing player
		if (Player == 0)
		{
			ChessBoard->TileMap[ChessBoard->BlackKing->PlaceAt]->SetTileColor(4);
		}
		else
		{
			ChessBoard->TileMap[ChessBoard->WhiteKing->PlaceAt]->SetTileColor(4);
		}
